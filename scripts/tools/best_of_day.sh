...
#          single summary post through the enabled push channels, attaching the
#          day's mosaics and timelapses (built by daily_imagery.sh, which this
#          also runs as a fallback in case a capture did not trigger a rebuild).
#
...
# --- attach the day's mosaics and timelapses ---
#
# these are rebuilt after every capture, so by now they already cover the whole
# day - this run is the fallback that catches days where a capture finished
...
if [ "${ENABLE_DAILY_TIMELAPSE}" == "true" ] || [ "${ENABLE_DAILY_MOSAIC}" == "true" ]; then
  ...
  shopt -s nullglob
  timelapses=("${IMAGE_OUTPUT}/timelapse-${day_stamp}-"*.gif)
  mosaics=("${IMAGE_OUTPUT}/mosaic-${day_stamp}-"*.jpg)
  shopt -u nullglob

  if [ "${#timelapses[@]}" -gt 0 ] || [ "${#mosaics[@]}" -gt 0 ]; then
    push_file_list="${push_file_list} ${timelapses[*]} ${mosaics[*]}"
    annotation="${annotation} | ${#timelapses[@]} timelapse(s), ${#mosaics[@]} mosaic(s)"
  fi
fi
